...
parser.add_argument('--save_dir', required=True, help='where to save the model')
...
# for i in range(2):
    audio = sf.read('./LA/ASVspoof2019_LA_eval/flac/'+train_list_bonafide[i])
...
# for i in range(2):
    audio = sf.read('./LA/ASVspoof2019_LA_eval/flac/'+train_list_spoof[i])
    sinc_filtered = sinc(torch.Tensor(pad_random(audio[0], 64600)).unsqueeze(dim=0))
    tensor = model(sinc_filtered.cuda())
    tensor2 = torch.squeeze(tensor.logits[:,int(time*len(tensor)/4),:],dim=0)
    tensor2 = tensor2.detach().cpu().numpy()
...
# for i in range(2):
    audio = sf.read('./LA/ASVspoof2019_LA_eval/flac/'+dev_list_bonafide[i])
    sinc_filtered = sinc(torch.Tensor(pad_random(audio[0], 64600)).unsqueeze(dim=0))
    tensor = model(sinc_filtered.cuda())
    tensor2 = torch.squeeze(tensor.logits[:,int(time*len(tensor)/4),:],dim=0)
    tensor2 = tensor2.detach().cpu().numpy()
...
# for i in range(2):
    audio = sf.read('./LA/ASVspoof2019_LA_eval/flac/'+dev_list_spoof[i])
    sinc_filtered = sinc(torch.Tensor(pad_random(audio[0], 64600)).unsqueeze(dim=0))
    tensor = model(sinc_filtered.cuda())
    tensor2 = torch.squeeze(tensor.logits[:,int(time*len(tensor)/4),:],dim=0)
    tensor2 = tensor2.detach().cpu().numpy()
...
full_save_dir = os.path.join(save_dir, 'iwax_sinc_') + str(low_freq) + '_' + str(high_freq) + '_' + str(time) + '_4.model'
